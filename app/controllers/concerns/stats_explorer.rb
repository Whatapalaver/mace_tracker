module StatsExplorer
  ...
  # Populates every ivar the stats explorer view needs. Pass locked_exercise (an Exercise) when
  # the caller shouldn't be able to pick a different one — the shared dashboard uses this when a
  # share link is scoped to a single exercise — otherwise equipment_id/exercise_id come from
  # params, exactly like the owner's own stats page.
  def build_stats_explorer(locked_exercise: nil)
    ...
    @signatures = stats_explorer_distinct_signatures
    ...
    @weights = stats_explorer_distinct_weights
    @weight = params[:weight].presence
    @output_labels = stats_explorer_output_labels
  ...
  def stats_explorer_structural_columns
    Progression::ComparabilityKey.structural_columns_for(@shape.name, granularity: @granularity)
  end

  def stats_explorer_distinct_signatures
    rows = Session.where(exercise_id: @exercise.id, session_shape_id: @shape.id)
                   .distinct.pluck(*stats_explorer_structural_columns)
    rows = rows.map { |row| stats_explorer_structural_columns.size == 1 ? [ row ] : row }
    rows.reject { |row| row.any?(&:nil?) }.uniq.sort.map do |row|
      { value: row.join(":"), label: stats_explorer_signature_label(row) }
    end
  end
  ...
  # filter is for).
  def stats_explorer_signature_label(row)
    case @shape.name
  ...
  # available.
  def stats_explorer_output_labels
